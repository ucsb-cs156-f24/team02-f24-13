...
describe("UCSBOrganizationIndexPage tests", () => {
  ...
  test("renders threeOrganizations correctly for regular user", async () => {
    setupUserOnly();
    axiosMock
    ...
    const orgCode = screen.getByText("cdt");
    expect(orgCode).toBeInTheDocument();

    const orgTranslationShort = screen.getByText("Chi Delts");
    expect(orgTranslationShort).toBeInTheDocument();

    const orgTranslation = screen.getByText("Chi Delta Theta");
  ...
  test("what happens when you click delete, admin", async () => {
    ...
    axiosMock
      ...
      .reply(200, "UCSBOrganization with orgCode cdt was deleted");
    ...
    await waitFor(() => {
      expect(mockToast).toBeCalledWith(
        "UCSBOrganization with orgCode cdt was deleted",
      );
    });
